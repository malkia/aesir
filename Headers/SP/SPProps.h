...
typedef struct SPPropertiesSuite {
	...
	/** Adds a set of properties to a plug-in properties list. This set is typically
		...
		but does not return these objects.
			...
			@see \c #AddProperty()
		*/
	SPAPI SPErr (*AddProperties)( SPPropertyListRef propertyList, PIPropertyList *pList, int refCon, int cacheable );
	...
	/** Creates a new individual property and adds it to a plug-in properties list.
		...
		*/
	SPAPI SPErr (*AddProperty)( SPPropertyListRef propertyList, PIType vendorID, PIType propertyKey, int propertyID, PIProperty *p,
				int refCon, int cacheable, SPPropertyRef *property );
	...
	/**	Retrieves a property from a plug-in properties list, or from any list in its chain.
		...
		*/
	SPAPI SPErr (*FindProperty)( SPPropertyListRef propertyList, PIType vendorID, PIType propertyKey, int propertyID, SPPropertyRef *property );
	...
	/**	Retrieves the current reference count of a property.
		...
		*/
	SPAPI SPErr (*GetPropertyRefCon)( SPPropertyRef property, int *refCon );
	/**	Reports whether a property is cacheable.
		...
		*/
	SPAPI SPErr (*GetPropertyCacheable)( SPPropertyRef property, int *cacheable );
	/**	Reports whether a property was allocated by the plug-in that contains it.
		...
		*/
	SPAPI SPErr (*GetPropertyAllocatedByPlugin)( SPPropertyRef property, int *allocatedByPlugin );
	...
	SPAPI SPErr (*FindPropertyLocal)( SPPropertyListRef propertyList, PIType vendorID, PIType propertyKey,
				int propertyID, SPPropertyRef *property );
...
/** Internal */
SPAPI SPErr SPAddProperties( SPPropertyListRef propertyList, PIPropertyList *pList, int refCon,
			int cacheable );

/** Internal */
SPAPI SPErr SPAddProperty( SPPropertyListRef propertyList, PIType vendorID, PIType propertyKey,
			int propertyID, PIProperty *p, int refCon, int cacheable, SPPropertyRef *property );
...
/** Internal */
SPAPI SPErr SPGetPropertyRefCon( SPPropertyRef property, int *refCon );
/** Internal */
SPAPI SPErr SPGetPropertyCacheable( SPPropertyRef property, int *cacheable );
/** Internal */
SPAPI SPErr SPGetPropertyAllocatedByPlugin( SPPropertyRef property, int *allocatedByPlugin );
...
SPAPI SPErr SPFindPropertyLocal( SPPropertyListRef propertyList, PIType vendorID,
			PIType propertyKey, int propertyID, SPPropertyRef *property );
