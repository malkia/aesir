...
@code
int myFlushCachesProc( const char *type, void *data, SPErr *error ) {
int flush = TRUE;
...
typedef struct {
/** File reference number, its not been used anywhere but its there for backwork compatibility */
short fileRefNum;
/** CF bundle */
...
typedef struct SPAccessSuite {
	...
	/** Retrieves the current reference count of an accessor object.
		...
		*/
	SPAPI SPErr (*GetAccessCount)( SPAccessRef access, int *count );
...
/** Internal */
SPAPI SPErr SPGetAccessCount( SPAccessRef access, int *count );
/** Internal */
